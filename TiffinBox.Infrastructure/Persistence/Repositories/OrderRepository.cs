    ...
    public class OrderRepository : GenericRepository<Order>, IOrderRepository
    {
        ...
        public async Task<IReadOnlyList<Order>> GetOrdersByVendorAsync(Guid vendorId, OrderStatus? status, DateTime? fromDate, DateTime? toDate)
        {
            ...
            if (fromDate.HasValue)
                query = query.Where(o => o.DeliveryDate >= fromDate.Value);

            if (toDate.HasValue)
                query = query.Where(o => o.DeliveryDate <= toDate.Value);
        ...
        public async Task<IReadOnlyList<Order>> GetFilteredAsync(OrderSpecification spec)
        {
            ...
            if (spec.CustomerId.HasValue)
                query = query.Where(o => o.Subscription.CustomerId == spec.CustomerId.Value);

            if (spec.VendorId.HasValue)
                query = query.Where(o => o.Subscription.VendorId == spec.VendorId.Value);

            if (spec.AgentId.HasValue)
                query = query.Where(o => o.DeliveryAgentId == spec.AgentId.Value);

            if (spec.Status.HasValue)
                query = query.Where(o => o.Status == spec.Status.Value);

            if (spec.FromDate.HasValue)
                query = query.Where(o => o.DeliveryDate >= spec.FromDate.Value);

            if (spec.ToDate.HasValue)
                query = query.Where(o => o.DeliveryDate <= spec.ToDate.Value);

            if (spec.IncludeItems)
            {
                query = query
                    .Include(o => o.OrderItems)
                    .ThenInclude(oi => oi.MenuItem);
            ...
            query = query.Include(o => o.Subscription)
                .ThenInclude(s => s.Plan)
                .ThenInclude(p => p.Vendor)
                .Include(o => o.DeliveryAgent)
                .ThenInclude(d => d.User);
